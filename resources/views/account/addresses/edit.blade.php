...
<div class="w-full bg-white">
    ...
    <div class="container mx-auto px-6 md:px-12 lg:px-20 max-w-7xl pt-32 pb-12">
        ...
        <p class="font-lora text-gray-600 ml-10">Update your delivery address</p>
    </div>

    <!-- Form Content -->
    <div class="container mx-auto px-6 md:px-12 lg:px-20 max-w-4xl pb-20">
        <form action="{{ route('account.address.update', $address) }}" method="POST" class="bg-neutral-50 rounded-sm p-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="country" 
                            ...
                            <option value="Philippines" {{ old('country', $address->country) == 'Philippines' ? 'selected' : '' }}>Philippines</option>
        ...
        <div class="mt-8 pt-8 border-t border-gray-200">
            <div class="bg-red-50 rounded-sm p-6">
                <h3 class="font-cg text-lg font-bold tracking-tight uppercase text-red-900 mb-2">
                    Danger Zone
                </h3>
                <p class="font-lora text-sm text-red-700 mb-4">
                    Once you delete this address, it cannot be recovered.
                </p>
                <form action="{{ route('account.address.destroy', $address) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this address? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
                            class="px-6 py-2 font-lora text-sm text-white bg-red-600 rounded-full hover:bg-red-700 transition-all duration-300">
                        Delete Address
                    </button>
                </form>
            </div>
